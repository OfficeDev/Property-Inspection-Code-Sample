...
namespace SuiteLevelWebApp.Controllers
{
    ...
    public class TestController : Controller
    {
        ...
        public async Task<ActionResult> GroupOneNote(string groupId = "GROUP GUID GOES HERE")
        {
            var graphService = await AuthenticationHelper.GetGraphServiceAsync();
            var oneNoteAccessToken = await AuthenticationHelper.GetOneNoteAccessTokenAsync();

            var group = await graphService.groups.GetById(groupId).ExecuteAsync();
            var groupSiteUrl = "https://TENANCY.sharepoint.com/sites/" + Server.UrlEncode(group.mailNickname);
            ...
            var getIdsUrl = string.Format(
                ...
                Server.UrlEncode(groupSiteUrl));
            var ids = await HttpGetAsync<JObject>(getIdsUrl, oneNoteAccessToken);

            var getNoteBooksUrl = string.Format(
                "{0}myOrganization/siteCollections/{1}/sites/{2}/notes/notebooks/?$filter=name eq '{3}'&$top=1",
                AADAppSettings.OneNoteResourceUrl,
                ids["siteCollectionId"],
                ids["siteId"],
                groupNoteBookName);
            var notebooksResult = await HttpGetAsync<JObject>(getNoteBooksUrl, oneNoteAccessToken);
            var notebooks = notebooksResult["value"] as JArray;
            var notebook = notebooks.First;

            var notebookId = notebook["id"].Value<string>();
            var notebookUrl = notebook["links"]["oneNoteWebUrl"]["href"].Value<string>();

            var html = string.Format("<h3>{0}</h3><p><b>ID: </b>{1}</p><a href='{2}'>{2}</a>", groupNoteBookName, notebookId, notebookUrl);
            return Content(html, "text/html");
        }

        private async Task<T> HttpGetAsync<T>(string url, string accessToken)
        {
            var client = new HttpClient();
