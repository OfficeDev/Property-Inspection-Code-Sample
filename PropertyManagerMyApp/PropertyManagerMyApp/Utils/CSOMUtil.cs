...
namespace SuiteLevelWebApp.Utils
{
    public class CSOMUtil
    {
        ...
        public static void AddUserToSPGroup(ClientContext clientContext, string LoginName)
        {
            int retrycount = 100;
            User user = null;
            while (retrycount-- > 0)
            {
                ...
                if (retrycount == 1)
                {
                    throw new Exception(string.Format("Can not resolve user {0}", LoginName));
                }
            }

            var collGroup = clientContext.Web.SiteGroups;
            var oGroup = collGroup.GetByName("Contoso Property Management Dashboard Members");
            User oUser = oGroup.Users.AddUser(user);
            clientContext.ExecuteQuery();
        }

        public static ListItem[] GetListItems(ClientContext clientContext, string listTitle)
        {
            var list = GetListByTitle(clientContext, listTitle);
