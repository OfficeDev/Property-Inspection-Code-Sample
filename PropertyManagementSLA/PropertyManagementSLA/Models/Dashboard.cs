...
namespace SuiteLevelWebApp.Models
{
    public class Dashboard
    {
        ...
        public async Task<DashboardInspectionDetailsViewModel> GetDashboardInspectionDetailsViewModel(int incidentId)
        {
            ...
            if (incident.sl_repairScheduled == null)
            {
                ...
                viewModel.repairPeople = (await GetRepairPeople()).ToList();
        ...
        public async Task<Event> ScheduleRepair(DashboardInspectionDetailsViewModel model)
        {
            ...
            updateJson.sl_repairPersonId = model.repairPeopleSelectedValue;
            ...
            var repairPeople = await GetRepairPeopleById(model.repairPeopleSelectedValue);
        ...
        private async Task<Inspection[]> GetInspectionsByPropertyId(int propertyId = 0)
        {
            string resturl = "/_api/lists/GetByTitle('Inspections')/Items?"
                + "$select=Id,sl_datetime,sl_finalized,sl_inspector/Id,sl_inspector/Title,sl_propertyID/Id,sl_propertyID/Title,sl_inspector/sl_emailaddress,sl_propertyID/sl_owner,sl_propertyID/sl_address1,sl_propertyID/sl_address2"
                + "&$expand=sl_inspector/Id,sl_inspector/Title,sl_inspector/sl_emailaddress,sl_propertyID/Id,sl_propertyID/Title,sl_propertyID/sl_owner,sl_propertyID/sl_address1,sl_propertyID/sl_address2";
        ...
        private async Task<RepairPeople[]> GetRepairPeople()
        {
        ...
        private async Task<RepairPeople> GetRepairPeopleById(int id)
        {
            string resturl = "/_api/lists/GetByTitle('Repair People')/Items?"
                + "$select=Id,Title,sl_accountname,sl_emailaddress"
                + string.Format("&$Filter=(Id eq {0})", id);
    ...
    public class Inspection
    {
        ...
        public Inspector sl_inspector { get; set; }
    ...
    public class ScheduleRepairUpdate
    {
        ...
        public DateTime sl_repairScheduled { get; set; }
        public int sl_repairPersonId { get; set; }
        public string sl_dispatcherComments { get; set; }
    }
