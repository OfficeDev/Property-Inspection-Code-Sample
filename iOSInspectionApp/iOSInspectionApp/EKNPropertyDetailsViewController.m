...
#pragma mark - UIAlertView/UIActionSheet/UIImagePicker/MFMailComposeViewController/MS graph
//after create an new incident item, we will send a email using graph
-(void)sendEmail: (NSString*)propertyId incidentId:(NSString*)incidentId type:(NSString *)type comment:(NSString *)comment
{
    ...
    if(tempdic!=nil)
    {
        ...
        if (to!=nil) {
            ...
            [emailDataDic setObject:self.loginName forKey:@"LogInUser"];
           ...
           // NSDictionary *insdic =(NSDictionary *)[tempitem getData:@"sl_inspector"];
            ...
            if(pdic!=nil)
            {
                if([[pdic objectForKey:@"ID"] intValue] == [pid intValue])
                {
                    ...
                    NSString *convertDateStr =[EKNEKNGlobalInfo converUTCStringFromDate:inspectiondatetime];
                    [inspectionItem setObject:convertDateStr forKey:@"sl_datetime"];
                    
                    if([inspectiondatetime compare:[NSDate date]] == NSOrderedDescending
                       ||[convertDateStr isEqualToString:[EKNEKNGlobalInfo converUTCStringFromDate:[NSDate date]]])
                    {
                        //upcoming
...
-(void)finalizeButtonClicked:(id)sender
{
    if(![self.commentViewSpinner isAnimating])
    {
        ...
        NSString *insid = [self getSelectLeftInspectionItemId];
        NSString *body = [NSString stringWithFormat:@"{'__metadata': { 'type': 'SP.Data.InspectionsListItem' }, 'sl_finalized':'%@'}",[EKNEKNGlobalInfo converUTCStringFromDate:[NSDate date]]];
            ...
            [self createIncidentItem:comment type:type ispectionId:insid propertyId:proId roomId:roomId callback:^(NSString *success)
             {
                 ...
                 {
                     NSString *incidentId = self.commentItemId;
...
-(void)createIncidentItem:(NSString *)comment type:(NSString *)type ispectionId:(NSString *)insIdstr propertyId:(NSString *)proId roomId:(NSString *)roomIdstr callback:(void (^)(NSString *success))callback
{
    ...
    NSMutableString *body =[[NSMutableString alloc] init];
    [body appendFormat:@"{'__metadata': { 'type': 'SP.Data.IncidentsListItem' }, 'sl_inspectionIDId':%d,'sl_roomIDId':%d,'sl_date':'%@','sl_propertyIDId':%d",[insIdstr intValue],[roomIdstr intValue],[EKNEKNGlobalInfo converUTCStringFromDate:[NSDate date]],[proId intValue]];
...
-(void)setLeftInspectionTableCell:(InspectionListCell *)cell cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(prodic!=nil)
    {
        ...
        if(inspecdic!=nil)
        {
            ...
            {
                if([[inspecdic objectForKey:@"sl_datetime"] isEqualToString:[EKNEKNGlobalInfo converUTCStringFromDate:[NSDate date]]])
                {
                    showPlus = YES;
                }
            }
...
-(void)getFirstPropertyId{
    ...
    [self.listClient getListItemsByFilter:@"Properties" filter:filterStr callback:^(NSMutableArray *listItems, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if(listItems != nil && [listItems count] >0)
            {
