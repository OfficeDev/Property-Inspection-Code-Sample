...
@implementation EKNListClient

const NSString *EKNApiUrl = @"/_api/lists";
...
- (void)getListItems:(NSString *)name callback:(void (^)(NSMutableArray *listItems, NSError *))callback{
    ...
    NSString *requestUrl = [NSString stringWithFormat:@"%@%@/GetByTitle('%@')/Items", self.Url , EKNApiUrl, [name urlencode]];
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
                                  {
                                     // NSLog(@"response %@",response);
                                      ...
                                      for (NSDictionary* value in listsItemsArray) {
                                          [array addObject: [[EKNListItem alloc] initWithDictionary:value]];
                                      }
...
- (void)getListItemsByFilter:(NSString *)name filter:(NSString *)filter callback:(void (^)(NSMutableArray *listItems, NSError *))callback{
    ...
    NSString *requestUrl = [NSString stringWithFormat:@"%@%@/GetByTitle('%@')/Items?%@", self.Url , EKNApiUrl, [name urlencode],filter];
   // NSLog(@"requestUrl %@",requestUrl);
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:requestUrl]];
    [request addValue:[NSString stringWithFormat:@"Bearer %@", self.token] forHTTPHeaderField:@"Authorization"];
    [request addValue:@"application/json;odata=verbose" forHTTPHeaderField:@"accept"];
...
- (void)getListItemFileByFilter:(NSString *)name FileId:(NSString *)fileId filter:(NSString *)filter callback:(void (^)(NSMutableArray *listItems, NSError *))callback{
    ...
    NSString *requestUrl = [NSString stringWithFormat:@"%@%@/GetByTitle('%@')/Items(%@)/File?%@", self.Url , EKNApiUrl, [name urlencode],fileId,filter];
   // NSLog(@"requestUrl %@",requestUrl);
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:requestUrl]];
    [request addValue:[NSString stringWithFormat:@"Bearer %@", self.token] forHTTPHeaderField:@"Authorization"];
    [request addValue:@"application/json;odata=verbose" forHTTPHeaderField:@"accept"];
...
{
    NSString *requestUrl = [NSString stringWithFormat:@"%@%@/GetByTitle('%@')/Fields/GetByTitle('%@')/%@", self.Url , EKNApiUrl, [listTitle urlencode],fieldTitle,[propertyName urlencode]];
