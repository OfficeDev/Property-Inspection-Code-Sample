...
{
    NSURL *url = [NSURL fileURLWithPath:[[NSBundle mainBundle] pathForResource:@"LeakyFaucet" ofType:@"mp4"]];
    NSData *data = [NSData dataWithContentsOfURL:url];
    
    [self uploadVideo:[NSString stringWithFormat:@"LeakyFaucet[%@]",incidentId] fileName:[EKNEKNGlobalInfo createFileName:@".mp4"] fileData:data description:@"" callback:^(SPVideoItem *videoItem, MSOrcError *error) {
        callback(error);
    }];
}
